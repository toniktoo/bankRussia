...
export const addSpaceValueDeposit = (number) => {
  return String(number).replace(/(\d)(?=(\d{3})+([^\d]|$))/g, "$1 ");
};

export const renderProfitSumm = (initSumm, timeDeposit, rate) => {
  return Math.trunc(((initSumm * (rate / 100)) / 365) * timeDeposit);
};
...
export const docCreator = (
  ...
  return {
    ...
    content: [
      {
        text: `Начальная сумма вклада: ${addSpaceValueDeposit(
          valueDeposit
        )} ₽.`,
      },
      {
